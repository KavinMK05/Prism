...
func buildZcodeModels(remap *config.ModelRemapping, cfg *config.Config) map[string]interface{} {
...
func InstallZcodeConfig(port int, remap *config.ModelRemapping) error {
	p := zcodeConfigPath()
	if p == "" {
	...
	models := buildZcodeModels(remap, config.Load())
