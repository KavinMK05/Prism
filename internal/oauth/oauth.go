// Package oauth provides OAuth2 flows (PKCE + device code) for Codex
// accounts, token refresh, and ChatGPT usage tracking.
package oauth
...
func RemoveOAuthAccount(id string) error {
	...
	found := false
	newAccounts := []*config.OAuthAccount{}
...
func getActiveOAuthAccount() *config.OAuthAccount {
	cfg := config.Current()
...
func getActiveOAuthAccountForConfig(cfg *config.Config) *config.OAuthAccount {
	for _, a := range cfg.OAuthAccounts {
...
func refreshAccessToken(account *config.OAuthAccount) error {
...
func GetValidAccessToken(account *config.OAuthAccount) (string, error) {
	oauthTokenMu.Lock()
