...
func exchangeCodeForTokens(flow *pendingOAuth, code string) (*config.OAuthAccount, error) {
...
func extractAccountDetails(account *config.OAuthAccount) {
...
func exchangeToken(account *config.OAuthAccount, params map[string]string) error {
	form := url.Values{}
...
func pollCodexDeviceToken(deviceCode string, interval time.Duration, timeout time.Duration) (*config.OAuthAccount, error) {
