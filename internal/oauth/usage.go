...
func RefreshUsageForAccount(account *config.OAuthAccount) (*UsageInfo, error) {
	usageMu.Lock()
...
func fetchCodexUsage(client *http.Client, accessToken string, accountID string) (*codexUsage, error) {
	req, err := http.NewRequest("GET", config.ChatGPTBase+"/backend-api/wham/usage", nil)
	if err != nil {
		return nil, err
	}
