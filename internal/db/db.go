// Package db provides SQLite persistence for request statistics.
package db
...
func getDBPath() string {
	return filepath.Join(platform.ConfigDir(), "stats.db")
}

// Init opens the stats database and applies the schema and migrations.
func Init() error {
	var err error
...
// Close closes the stats database.
func Close() {
	if db != nil {
		db.Close()
		db = nil
...
// RecordRequest inserts a completed request into the requests table.
func RecordRequest(req RequestStats) error {
	if db == nil {
		return nil
	}
	_, err := db.Exec(
...
// RecordTPS inserts a TPS snapshot into the tps_snapshots table.
func RecordTPS(model, provider, client string, tps float64) error {
	if db == nil {
		return nil
	}
	_, err := db.Exec(
...
// GetDailyTokens returns per-day token totals, optionally filtered.
...
// GetTPSHistory returns TPS bucketed into 5-minute windows, optionally filtered.
...
// ClearAllStats deletes all rows from both tables.
...
// GetDistinctProviders returns all distinct provider IDs seen in requests.
...
// GetClientHistory returns per-client aggregation, optionally filtered.
...
// GetDistinctClients returns all distinct client names seen in requests.
