...
// §4). It renders a templated HTTP request from a CustomProviderConfig,
...
func (cp *CustomProviderConfig) needsKey() bool {
	return cp != nil && cp.AuthHeader != ""
}

func newCustomSearchProvider(cp *CustomProviderConfig, client *http.Client) SearchProvider {
...
func (p *customSearchProvider) mapItem(item interface{}) (SearchResult, bool) {
	...
	if v, ok := field("highlights"); ok {
		if arr, ok := v.([]interface{}); ok {
			...
			if res.Snippet == "" {
				res.Snippet = util.JoinStrings(res.Highlights)
			}
...
func MergeCustomProviderInput(existing []*CustomProviderConfig, incoming []AdminCustomProviderInput) ([]*CustomProviderConfig, error) {
	byID := map[string]*CustomProviderConfig{}
