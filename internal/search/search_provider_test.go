...
func TestMergeConfigFillsDefaults(t *testing.T) {
	c := &Config{Active: "exa"} // minimal, missing providers/limits
	got := MergeConfig(c)
	if got.MaxPerTurn != 5 {
...
func TestSearchRunnerFallbackChain(t *testing.T) {
	...
	searchRegistry = map[string]func(pc *ProviderConfig, client *http.Client) SearchProvider{
		"a": func(_ *ProviderConfig, _ *http.Client) SearchProvider {
			return &fakeProvider{id: "a", results: nil, err: errSearchAllFailed("a down")}
		},
		"b": func(_ *ProviderConfig, _ *http.Client) SearchProvider {
...
func TestSearchRunnerSkipsUnconfiguredKeyProviders(t *testing.T) {
	...
	called := false
	searchRegistry = map[string]func(pc *ProviderConfig, client *http.Client) SearchProvider{
		"needskey": func(_ *ProviderConfig, _ *http.Client) SearchProvider {
			called = true
			return &fakeProvider{id: "needskey", needsKey: true, results: []SearchResult{{URL: "https://x"}}}
		},
		"free": func(_ *ProviderConfig, _ *http.Client) SearchProvider {
...
func TestAdminConfigViewMasksKeys(t *testing.T) {
	c := &Config{
		Active:            "searxng",
		MaxPerTurn:        5,
		TimeoutMs:         8000,
