// Package stats provides in-memory request statistics tracking.
package stats
...
type StatsTracker struct {
	mu                 sync.Mutex
	recentRequests     []db.RequestStats
	maxRecent          int
	totalRequests      int
...
var Global = NewStatsTracker(50)

// NewStatsTracker creates a stats tracker that keeps maxRecent recent requests.
func NewStatsTracker(maxRecent int) *StatsTracker {
	return &StatsTracker{
		recentRequests: make([]db.RequestStats, 0, maxRecent),
		maxRecent:      maxRecent,
...
func (st *StatsTracker) GetSnapshot() LiveStats {
	...
	recent := make([]db.RequestStats, len(st.recentRequests))
...
// StatsToJSON returns the global stats as JSON bytes
func StatsToJSON() ([]byte, error) {
	snapshot := Global.GetSnapshot()
