...
type ProviderRouter struct {
	cfg        *config.Config
	cfgMu      sync.RWMutex
	modelRemap atomic.Pointer[config.ModelRemapping]
	client     *http.Client
}

func NewRouter(cfg *config.Config, modelRemap *config.ModelRemapping) *ProviderRouter {
	pr := &ProviderRouter{
		cfg:    cfg,
...
func (pr *ProviderRouter) ReloadModelRemapping() {
	pr.modelRemap.Store(config.LoadModelRemapping())
}
...
func (pr *ProviderRouter) getConfig() *config.Config {
	pr.cfgMu.RLock()
...
func (pr *ProviderRouter) resolveProviderForModel(requestedModel string) (*config.ResolvedProvider, string, error) {
	cfg := pr.getConfig()
	resolvedModel, providerID := config.ResolveModelProvider(cfg, pr.getModelRemap(), requestedModel)
	...
	if providerInfo.ProviderType == "codex" {
		for _, a := range cfg.OAuthAccounts {
			if a.ID == providerID {
				token, err := oauth.GetValidAccessToken(a)
				if err != nil {
				...
				if accountID == "" && token != "" {
					accountID = oauth.ParseChatGPTAccountID(token)
				}
